...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Bank of Israel public API');
}
...
const tools: McpToolExport['tools'] = [
  ...
  {
    name: 'boi_interest_rate',
    description:
      'The current Bank of Israel (BOI) policy interest rate for ISRAEL — the Israeli benchmark rate set by the Bank of Israel Monetary Committee, in % per annum. Returns currentInterest (the Israeli policy rate), nextInterestDate (next Israeli rate decision), and lastPublishedDate. Scope: Israel only.',
    inputSchema: { type: 'object', properties: {} },
  },
];
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    case 'boi_exchange_rates':
      return boiGet('/GetExchangeRates?asJson=true');
    case 'boi_exchange_rate': {
      const key = reqStr(args, 'key', '"USD" or "EUR"').trim().toUpperCase();
      try {
        return await boiGet(`/GetExchangeRate?key=${encodeURIComponent(key)}&asJson=true`);
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        if (/\b404\b/.test(msg)) {
          throw new Error(
            `Bank of Israel: no exchange rate for currency "${key}" (HTTP 404). Call boi_exchange_rates for the full list of currencies BOI publishes.`,
          );
        }
        throw err;
      }
    }
    case 'boi_interest_rate':
